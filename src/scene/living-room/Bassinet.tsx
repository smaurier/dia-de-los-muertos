...
// The blanket breathes gently — the only movement, and it is enough.
...
export function Bassinet({ position }: { position: [number, number, number] }) {
  ...
  useFrame((_, delta) => {
    ...
    if (blanketRef.current) {
      // Sleeping infant breathing: slow, barely 2% amplitude
  ...
  return (
    <group position={position} rotation={[0, 0.4, 0]}>
      {/* Oval wicker basket (open sides) + base */}
      ...
      {/* Wicker band (woven reinforcement) */}
      ...
      {/* Mattress + small sleeping head + breathing blanket */}
